...
public class Reports_RetrivalRequestPage extends BaseClass {
	...
	/*----------------------Satrt of Fednow Methods--------------------------------------------------------------------*/	
	/* FedNow Flow Methods */

	public void FednowRRreport() throws InterruptedException	
	{
		selectoptionfromList(SelectmessageDropdown, Messageslist, prop.getProperty("FednowMessage"));
	...
	public void FedwireRRreport() throws InterruptedException	
	{
		selectoptionfromList(SelectmessageDropdown, Messageslist, prop.getProperty("FedwireMessage"));
		//	    JSClick(driver,closedropdownusingLabel,"Clicking outside dropdown to close.");

		Actions actions = new Actions(driver);
		actions.sendKeys(Keys.ESCAPE).perform();

		selectoptionfromList(FlowType, FlowTypevalue, prop.getProperty("FedwireFlowType"));

		sendkeys(seqFromNum,prop.getProperty("seqFromNum"));
		sendkeys(seqToNum,prop.getProperty("seqToNum"));

		clickelementwithname(routingNumber, "Opening routing number dropdown");
		routingnumberselect(routingNumbervalueinput,prop.getProperty("RoutingNumber"));

		scrollByVisibilityOfElement(driver,viewMessageBtn);
		JSClick(driver,RRreport.viewMessageBtn,"Clicking View Message button");

		visibleofele(driver, RetrievalRequestMessagesScreenHeader, "Retrieval Request Messages Screen Header");

		Thread.sleep(1000);
		JSClick(driver,messagesScreenCheckbox, "Selecting checkbox on Messages Screen.");
